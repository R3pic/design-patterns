import { Singleton } from "src/design-patterns/creational-patterns/singleton-pattern/broke-example/serialize-example/singleton.ts";
import {ExampleRunner} from '@example-runner';

export default class SingletonSerializeBrokenExampleRunner extends ExampleRunner {
  static name = '싱글턴 패턴 직렬화 역직렬화 취약점 예제';
  ...
  run() {
    ...
    const deserializedInstance = JSON.parse(serialized);
...
// Date 객체 손실 문제 보여주기
    ...
    this.logger.log(`- 역직렬화 후 createdAt은 문자열: ${typeof deserializedInstance.createdAt === "string"}`);
  }
}